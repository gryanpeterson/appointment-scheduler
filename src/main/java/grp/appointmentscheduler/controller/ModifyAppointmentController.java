...
import grp.appointmentscheduler.controller.AppointmentsController;
...
/** The modify appointment controller class.
 *
 */
public class ModifyAppointmentController implements Initializable {
    Stage stage;
    Parent scene;
    /** The object selected by the user in the appointment controller.
     *
     */
    ...
    void onActionSave(ActionEvent event) throws IOException {
        ...
        if (titleTextField.getText().isEmpty() || descriptionTextField.getText().isEmpty() || locationTextField.getText().isEmpty() || typeTextField.getText().isEmpty() || contactComboBox.getValue() == null || customerIDTextField.getText().isEmpty() || userIDTextField.getText().isEmpty() || startDateDatePicker.getValue() == null || endDateDatePicker.getValue() == null || startTimeTextField.getText().isEmpty() || endTimeTextField.getText().isEmpty()) {
            ...
            LocalDateTime newStart = startLDT;
            LocalDateTime newEnd = endLDT;
            ...
            if (Helper.isOutsideBusinessHours(startLDT)) {
                ...
                for (Appointment appointment : AppointmentDAO.getAllAppointments()) {
                    ...
                    if (customerID == appointment.getCustomerID() && appointmentID != appointment.getAppointmentID()) {
                        // Start In Window
                        if ((newStart.isAfter(dbStart) || newStart.isEqual(dbStart)) && newStart.isBefore(dbEnd)) {
                            Alert alert = new Alert(Alert.AlertType.ERROR);
                            alert.setTitle("Error Scheduling Appointment");
                        ...
                        // End in Window
                        if ((newEnd.isBefore(dbEnd) || newEnd.isEqual(dbEnd)) && newEnd.isAfter(dbStart)) {
                            Alert alert = new Alert(Alert.AlertType.ERROR);
                            alert.setTitle("Error Scheduling Appointment");
                            alert.setHeaderText("Appointment Overlap");
                            alert.setContentText("New appointment date and time for Customer ID: " + customerID + " overlaps with existing Appointment ID: " + appointment.getAppointmentID());
                        ...
                        // Start & End in Window
                        if ((newStart.isBefore(dbStart) || newStart.isEqual(dbStart)) && (newEnd.isAfter(dbEnd) || newEnd.isEqual(dbEnd))) {
                            Alert alert = new Alert(Alert.AlertType.ERROR);
                            alert.setTitle("Error Scheduling Appointment");
                            alert.setHeaderText("Appointment Overlap");
                            alert.setContentText("New appointment date and time for Customer ID: " + customerID + " overlaps with existing Appointment ID: " + appointment.getAppointmentID());
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        selectedAppointment = AppointmentsController.getAppointmentToModify();

        int contactID = selectedAppointment.getContactID();
        String contactName = Helper.contactIDToContactName(selectedAppointment.getContactID());
        Contact selectedContact = new Contact(contactID, contactName);
